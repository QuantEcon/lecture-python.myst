...
They detect a defects in their model, one of which {cite:t}`MehraPrescott1985` later  called   the **equity premium puzzle**.
...
Hansen and Singleton estimated their model by {doc}`mle`.

  

The empirical findings of {cite:t}`hansen1983stochastic` constitute  what {cite:t}`MehraPrescott1985` would later call the **equity premium puzzle**.

To keep lecture this lecture narrowly focused, we estimate one return at a time (either a market proxy or a T-bill) rather than the  full multi-asset systems studied by  {cite:t}`hansen1983stochastic`.
...
We now apply the maximum likelihood estimator of {cite:t}`hansen1983stochastic` to real data.
...
Our estimates reproduce the pattern that {cite:t}`MehraPrescott1985` later called  the **equity premium puzzle**.
...
{cite:t}`MehraPrescott1985` presented closely related findings. 
...
A companion lecture {doc}`hansen_singleton_1982` describes a closely related paper in which
