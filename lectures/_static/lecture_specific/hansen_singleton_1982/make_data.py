#!/usr/bin/env python3
"""
Build the vendored dataset for the ``hansen_singleton_1982`` lecture.

Downloads the raw inputs from FRED and the Ken French data library, constructs
the monthly gross real return and gross consumption-growth series used in the
lecture, and writes them to ``hansen_singleton_1982_data.csv`` next to this
script.
...
START = "1959-02-01"
END = "1978-12-01"
OUTPUT = Path(__file__).with_name("hansen_singleton_1982_data.csv")
...
def build(start=START, end=END):
    ...
    frame = out[["gross_real_return", "gross_cons_growth"]].copy()
