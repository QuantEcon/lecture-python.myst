...
{doc}`doubts_or_variability`) motivate the alternative approach
...
4. *Distorted beliefs*: reinterpreting risk price estimates when agents hold systematically
   biased forecasts ({cite:t}`piazzesi2015trend`); see also {doc}`risk_aversion_or_mistaken_beliefs`.
...
Our {doc}`risk_aversion_or_mistaken_beliefs` lecture
...
{doc}`doubts_or_variability` and to robust
