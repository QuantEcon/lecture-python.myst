...
theories of {cite}`Friedman1956` and {cite}`Hall1978`.  We use the model to illustrate:
...
(HST) and {cite}`hansen2008robustness`.  A consumer who distrusts his specification of the labor income process engages in a form of
precautionary savings even when preferences are quadratic.  We show:
...
concludes by combining Parts A and B in a single general-equilibrium model.  Using tools that we borrow from {cite}`hansen2008robustness`, we show:
...
where $E_t$ is the mathematical expectation conditioned on the consumer's time-$t$ information,
...
where $\gamma > 0$ is a bliss level of consumption.  We allow $c_t$ to be negative (a producer
...
$$
b_t = \sum_{j=0}^{\infty} \beta^j E_t y_{t+j} - \frac{1}{1-\beta} c_t
$$ (eq:sprob7)
...
$$
c_{t+1} - c_t = (1-\beta)\sum_{j=0}^{\infty} \beta^j \bigl(E_{t+1} y_{t+j+1} - E_t y_{t+j+1}\bigr)
$$ (eq:sprob11)
...
We illustrate formulas {eq}`eq:sprob16` with two examples.  In both, the endowment is
...
can reconstruct $w_{t+1}$ from $z_{t+1}$ and $z_t$.  Applying {eq}`eq:sprob16`:
...
and causes **zero net saving**.  A unit increment to the purely transitory component raises
...
but not $z_{1t}$ and $z_{2t}$ separately.  The appropriate approach uses an **innovations
...
its own innovation $a_t = y_t - E[y_t \mid y^{t-1}]$ (the one-step-ahead forecast error):
...
$K$ of last period's surprise was treated as permanent; the remainder mean-reverts.  The scalar
...
as transitory.  He permanently increments consumption by $K + (1-\beta)(1-K) = 1 - \beta(1-K)$
...
Consider a continuum of *ex ante* identical households born at $t = 0$.  Each household $i$ has
the same preferences and the same stochastic income process, but faces **idiosyncratic** shocks
$w_{t+1}^i$.  Let all households start from the same initial conditions $c_0^i = c_0$ and $z_0^i$.
...
$$
E_0\bigl(c_t^i - c_0^i\bigr)^2 = t\, h h'
$$ (eq:varspread)

In the two-factor model, $h$ is a $1 \times 2$ row vector so $hh'$ is a positive scalar equal to
$\sigma_1^2 + (1-\beta)^2\sigma_2^2$.  The cross-section variance of consumption grows like $t$.
...
for t in range(T_sim):
    eps = np.random.randn(N, 2)    # N draws of 2D shock
...
ax.set_title('Spreading Consumption Cross Sections')
ax.legend()
ax.grid(alpha=0.3)
plt.show()
```
...
economy").  Following ideas of {cite}`Bewley1977`, we can construct a **closed economy** in which
$R = \beta^{-1}$ is an **equilibrium outcome**.

**Environment.** A continuum of measure one of consumers, indexed by $i \in [0,1]$, trade a
risk-free one-period bond with price $\beta$.  All consumers have the same preferences and the
same stochastic income process, but face **idiosyncratic** income shocks.  Initial bond positions
are zero: $b_0^i = 0$ for all $i$.  Initial endowment states $z_0^i$ are independent draws from
...
$$
\int_0^1 c_0^i\, di = (1-\beta)\sum_{j=0}^{\infty}\beta^j E_0\!\int_0^1 y_j^i\, di = Y
$$ (eq:c_marketclear_0)

because the continuum of idiosyncratic shocks averages out.  For future periods, integrating
{eq}`eq:c_future`:
...
The goods market clears at every date at **constant** aggregate consumption equal to $Y$.  The
...
# store the full N×T consumption array in memory.
np.random.seed(0)
...
z0_i   = np.random.randn(N_bew, 2) * np.array([1.0, σ2])
...
distinct from the usual precautionary motive (which requires a convex marginal utility).  Here, the
...
$\beta$).  We develop this result carefully below.
...
$$
-x' P x - p =
\sup_c \inf_w \Bigl\{-(s-b)^2 + \beta\bigl(\theta w^{*\prime} w^* - E\,x^{*\prime} P x^* - p\bigr)\Bigr\}
$$ (eq:income1)
...
$$
E_0\sum_{t=0}^{\infty}\beta^t\bigl\{-(s_t - b_t)^2\bigr\}
$$ (eq:income5)
...
\mu_{ht} &= \beta E_t[\delta_h \mu_{h,t+1} - \lambda \mu_{s,t+1}] \\
\mu_{ct} &= \beta R\, E_t\mu_{c,t+1}
\end{aligned}
$$ (eq:foc)
...
$E_t\mu_{c,t+1} = (\beta R)^{-1}\mu_{ct}$, so $\mu_{st}$ satisfies a **martingale representation**
when $\beta R = 1$:
...
$$
\mu_{st} = \Psi_1 k_{t-1} + \Psi_2 h_{t-1} + \Psi_3\sum_{j=0}^{\infty} R^{-j} E_t b_{t+j}
            + \Psi_4\sum_{j=0}^{\infty} R^{-j} E_t d_{t+j}
$$ (eq:income10)
...
equals that out of **human wealth** $\sum_{j=0}^{\infty}R^{-j}E_t d_{t+j}$—a well-known feature of
the LQ model.
...
$\hat\beta R\,\hat E_t\mu_{s,t+1} = \mu_{st}$ to hold under the worst-case model, which gives
...
```{code-cell} ipython3
# Observational equivalence locus: β̂(σ) = 1/R + σα² / (R-1)
# HST benchmark values
β0    = 0.997          # benchmark discount factor
R0    = 1.0 / β0       # gross interest rate
α2_vals = [0.005, 0.015, 0.030]   # three values of α² for illustration
...
ax.set_title('Observational Equivalence Locus\n'
             r'$\hat\beta(\sigma) = R^{-1} + \sigma\alpha^2/(R-1)$')
ax.legend()
ax.grid(alpha=0.3)
plt.show()
```
...
$$
c_t = (1 - R^{-2}\beta^{-1})\!\left[Rk_{t-1} + E_t\sum_{j=0}^{\infty}R^{-j}d_{t+j}\right]
      + \left(\frac{(R\beta)^{-1}-1}{R-1}\right)\!b
$$ (eq:consfunction)

The **marginal propensity to consume** out of non-human wealth $Rk_{t-1}$ **equals** that out of
human wealth $E_t\sum R^{-j}d_{t+j}$.  This equal-propensity property is a hallmark of the LQ
...
the Euler equation implies $E_t c_{t+1} > c_t$: expected consumption grows over time.  Thus,
...
$$
u'''(c) > 0 \;\Rightarrow\; E_t u'(c_{t+1}) > u'(E_t c_{t+1}) \;\Rightarrow\; E_t c_{t+1} > c_t
$$
...
# With β R = 1, robust consumer behaves like σ=0 consumer with β̂R = 1 + ε > 1
# Euler equation: E_t c_{t+1} = (β̂R) c_t  → upward drift

β_vals  = [0.990, 0.993, 0.997]     # σ < 0 ↔ β̂ < β0 in Theorem 1
R_fixed = 1 / 0.997
T_plot  = 40
...
ax.set_title('Expected Consumption Profile\n'
             r'Under Different Observational Equivalence $(σ, \hat β)$ Pairs')
...
The distorted expectations operator $\hat E_t$ satisfies

$$
\hat E_t X_{t+j} = (A - BF(\sigma,\hat\beta) + CK(\sigma,\hat\beta))^j X_t
$$

Observational equivalence requires that the modified human-wealth formula

$$
\hat\Psi_4 \sum_{j=0}^{\infty} R^{-j}\hat E_t d_{t+j}
$$

equals its benchmark counterpart $\Psi_4 \sum_{j=0}^{\infty} R^{-j} E_t d_{t+j}$.  This is
achieved by a mutual adjustment of the coefficients $\hat\Psi_j$ (via $\hat\beta$) and the
distorted expectation operator $\hat E_t$ (via $\sigma$).
...
# Under worst-case model: effective AR coefficient = ρ + α*K(σ, β̂)
...
for σ_rob in σ_rob_vals:
    # α K ≈ |σ| * P * α² / (1 - |σ| * α² * P) — simplified scalar version
    # For illustration, use a linear approximation: Δρ ≈ -σ * α² * const
    α2_scal  = 0.01
...
ax.set_title('Worst-Case Model Makes Endowment\nMore Persistent')
ax.legend(fontsize=9)
ax.grid(alpha=0.3)
plt.show()
```
...
# Model as a low-pass spectrum W(ω) ∝ 1/(ω² + κ²)
...
$$
\text{DEP}(\sigma) = \frac{1}{2}\bigl[\Pr(\text{prefer approx.} \mid \text{worst-case is true})
                                    + \Pr(\text{prefer worst-case} \mid \text{approx. is true})\bigr]
$$
...
α2_val   = 0.01       # representative α² value
R_dep    = 1.0 / 0.997

# For each σ compute: K(σ,β̂) from worst-case multiplier, then Δ, then DEP
def compute_dep(σ_val, α2, R, T):
    ...
    # Scalar Bellman equation for P(β̂)
    ...
    # KL divergence (one-sided) approximation per period: 0.5 * K² * α²
...
ax.set_title('Detection Error Probabilities vs Robustness Parameter\n'
             rf'($T = {T_sample}$ quarters, $\alpha^2 = {α2_val}$)')
ax.legend()
ax.grid(alpha=0.3)
plt.show()
```
...
$$
\pi(\sigma_1;\sigma_2) = -E_{0,\sigma_1}\sum_{t=0}^{\infty}\beta^t\, x_t' H(\sigma_2)' H(\sigma_2)\, x_t
$$ (eq:soln3)
...
# π(σ₁; σ₂) measures performance of rule σ₂ when model σ₁ generates data
...
def payoff_approx(σ1, σ2, α2, β, R):
    """
    Approximate payoff π(σ₁; σ₂) in a stylised scalar version.
    Captures the key qualitative pattern from HST.
    """
    # Effective drift under (F(σ₂), K(σ₁)):
    ...
    # Effective persistence of state under (F(σ₂), K(σ₁))
...
ax.set_title(r'Payoff $\pi(\sigma_1;\sigma_2)$ as function of data-generating $\sigma_1$')
ax.legend(fontsize=9)
ax.grid(alpha=0.3)
plt.show()
```
...
```{code-cell} ipython3
# Compute the Theorem 2 equivalence: β̃(σ̂) vs σ̂
# Starting from β̂ R = 1 and σ̂ < 0

β_hat_val  = 0.997     # β̂ satisfying β̂R = 1
α2_th2     = 0.01

σ_hat_grid = np.linspace(-0.001, 0.0, 300)

def β_tilde(σ_hat, β_hat, α2):
    """Theorem 2 formula for β̃."""
...
ax.set_title('Theorem 2: Observational Equivalence Locus\n'
             r'$\sigma = 0, \tilde\beta > \hat\beta$ replicates $(\hat\sigma, \hat\beta)$ allocation')
ax.legend()
ax.grid(alpha=0.3)
plt.show()
```
...
The objective is $E_0 \sum_{t=0}^\infty \beta^t [-(c_t - \gamma)^2/2]$, which is the HST
...
```{code-cell} ipython3
# ── Part B Bewley model: compute α² and the observational equivalence locus ──
# Reuse Part A parameters (β = 0.95, σ1 = 0.15, σ2 = 0.30 from the top of the lecture).
# Note: Part B's HST calibration used β₀ = 0.997; here we deliberately re-use the
...
# α² for the two-factor Bewley model  (eq:bew_alpha2)
α2_bew = σ1_bew**2 + (1 - β0_bew)**2 * σ2_bew**2

print(f"α²  (Bewley, two-factor)        = {α2_bew:.6f}")
print(f"  permanent component  σ₁²     = {σ1_bew**2:.6f}  "
      f"({100*σ1_bew**2/α2_bew:.1f} % of α²)")
print(f"  transitory component (1-β)²σ₂²= {(1-β0_bew)**2*σ2_bew**2:.6f}  "
      f"({100*(1-β0_bew)**2*σ2_bew**2/α2_bew:.1f} % of α²)")
```

```{code-cell} ipython3
# ── Observational equivalence locus  β̂(σ) = β₀ + σ α² β₀ / (1-β₀) ──────────
# σ range: go down until β̂ = 0.80
...
# ── Right: α² decomposition as σ₁ and σ₂ vary ────────────────────────────────
...
# h = [σ₁, (1-β₀)σ₂]  (from eq:bew_cinno for the two-factor model)
h_bew = np.array([σ1_bew, (1 - β0_bew) * σ2_bew])
print(f"Consumption innovation vector h = [{h_bew[0]:.4f}, {h_bew[1]:.4f}]")
print(f"Var(Δc) = h·h' = α² = {h_bew @ h_bew:.6f}  (equals α²_bew: {α2_bew:.6f})")

np.random.seed(17)
T_het = 80
eps   = np.random.randn(T_het, 2)    # one common shock sequence
...
# Simple scalar illustration: ρ_wc ≈ 1 + √α² · K(σ^i, β^i), K from Bellman eqn
...
for i, (s, b) in enumerate(zip(σ_types, β_types)):
    if s == 0.0:
        ...
        # effective AR root of z_{1t} under worst-case: 1 + √α² · K
